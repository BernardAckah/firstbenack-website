...
app.use(function (req, res, next) {
    ...
    res.locals.filterUserHTML = function (content) {
        return sanitizeHTML(marked.parse(content), {
            allowedTags: ["p", "br", "ul", "li", "stromg", "bold", "i", "em", "h1", "h2", "h3", "h4", "h5", "h6"],
            allowedAttributes: {}
        })
    }
...
function sharedPostValidation(req) {
    ...
    // if(req.body.title !== req.body.title) req.body.title = errors.push("Enter a psot")
...
app.post("/login", (req, res) => {
    ...
    const ourTokenValue = jwt.sign({
        ...
        skycolor: "blue", userid: userInQuestion.id, username: userInQuestion.username
    },
...
app.post("/register", (req, res) => {
    ...
    if (req.body.username && req.body.username.length < 3) errors.push("Username should be more than 3 characters")
    if (req.body.username && req.body.username.length > 12) errors.push("Username should not exceed more than three charaters")
    // if (req.body.username && !req.body.username.match(/^ [a-zA-Z0-9]+$/)) errors.push("Username can only contain letters and numbers")

    // check if username exist already
    const usernameStatement = db.prepare("SELECT * FROM users WHERE USERNAME = ?")
    const ourNameCheck = usernameStatement.get(req.body.username)

    if (ourNameCheck) errors.push("That username has been taken")

    if (req.body.password && req.body.password.length < 5) errors.push("password should be more than 5 characters")
    if (req.body.password && req.body.password.length > 20) errors.push("password should not exceed more than three charaters")
    ...
    const ourTokenValue = jwt.sign({
        exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24,
        skycolor: "blue", userid: ourUser.id, username: ourUser.username
    },
        process.env.JWTSECRET)
